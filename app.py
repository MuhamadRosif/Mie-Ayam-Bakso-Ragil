...
def user_page():
    ...
    with col_menu:
        ...
        if st.button("Tambah ke Keranjang",disabled=disable_order):
            ...
            save_json(CHECKOUT_FILE,checkout); st.success("✅ Ditambahkan ke keranjang"); st.rerun()
    with col_cart:
        st.subheader("🧾 Keranjang Saat Ini")
        with st.expander("Lihat Keranjang"):
            if checkout: df=pd.DataFrame(checkout); df["total"]=df["harga"]*df["jumlah"]; st.dataframe(df,width="stretch")
            else: st.info("Keranjang kosong")
    if not st.session_state.admin_logged:
        if st.sidebar.button("Admin Login"): st.session_state.page="admin_login"; st.rerun()
...
def admin_login_page():
    ...
    if st.button("Kembali ke Menu User"): st.session_state.page="user"; st.rerun()

def admin_page():
    st.sidebar.title("⚙️ Admin Panel")
    menu=st.sidebar.radio("Menu Admin", ["Data Menu","Data Pesanan","Pembayaran","Laporan","Pengaturan Toko"])
    if st.sidebar.button("Logout Admin"): st.session_state.admin_logged=False; st.session_state.page="user"; st.rerun()
    ...
    if menu=="Data Menu":
        ...
        if st.button("Simpan Menu"): menu_data[kat][nama]=harga; save_json(MENU_FILE,menu_data); st.success("✅ Menu disimpan"); st.rerun()
        ...
        if st.button("Hapus Menu"): del menu_data[del_kat][del_item]; save_json(MENU_FILE,menu_data); st.success("✅ Menu dihapus"); st.rerun()
